...
import { Innertube, YTNodes } from 'youtubei.js/cf-worker';
const corsHeaders = {
...
export default {
	async fetch(request, env, ctx): Promise<Response> {
		...
		if (url.pathname.indexOf("/youtubejs/proxy")>= 0) {
...
async function proxy(request: Request<unknown, IncomingRequestCfProperties<unknown>>): Promise<Response> {
